...
export default function TabelaDeTickets({ onViewTicket, refreshTrigger }) {
	...
	useEffect(() => {
		...
		async function loadInitialData() {
			...
			try {
				...
				const ticketQuery = statusFilter
				...
				if (isMounted) {
					const ticketsData =
						ticketsPayload?.data || ticketsPayload || [];
					const sortedTickets = Array.isArray(ticketsData)
						? ticketsData.sort(
								(a, b) =>
				...
				if (categoriesRes.ok) {
					...
					if (isMounted) {
						const categoriesData =
							categoriesPayload?.data || categoriesPayload || [];
						setCategories(
				...
				if (patrimoniesRes.ok) {
					...
					if (isMounted) {
						const patrimoniesData =
							patrimoniesPayload?.data ||
							patrimoniesPayload ||
							[];
						setPatrimonies(
							Array.isArray(patrimoniesData)
								? patrimoniesData
								: []
						);
					}
	...
	}, [authHeaders, statusFilter, handleApiError, refreshTrigger]);
	...
	const renderCell = (item, columnKey) => {
		switch (columnKey) {
				...
				return (
					<div className="flex justify-center">
						{item.patrimony_code ? (
							...
							<span className="text-zinc-400 dark:text-zinc-500 italic text-sm">
								N/A
							</span>
						)}
					</div>
			...
			case 'status':
				...
				const statusStyles = {
					Pendente:
						'bg-red-500/20 border border-red-500 text-white w-30',
					'Em Andamento':
						'bg-orange-500/20 border border-orange-500 text-white w-30',
					Concluído:
						'bg-green-500/20 border border-green-500 text-white w-30',
				};
				return (
			...
			case 'title':
				return (
					<div className="text-center flex items-center justify-center">
						<p
							className="font-medium text-zinc-900 dark:text-zinc-100 text-sm max-w-[200px] truncate"
							title={item.title}
						>
