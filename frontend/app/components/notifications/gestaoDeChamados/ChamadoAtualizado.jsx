...
import { CircleAlert, CircleFadingArrowUp, X } from "lucide-react";

export default function ChamadoAtualizado({ show, onClose, title, description }) {
    if (!show) return null;
            ...
            <div className="flex flex-col gap-2 w-60 sm:w-72 text-[10px] sm:text-xs mt-10
                ...
                <div className="flex items-center justify-between w-full h-12 sm:h-14">
                    <div className="flex gap-2">
                        <div className="text-blue-600 bg-zinc-500/20 backdrop-blur-xl p-1 rounded-lg">
                            <CircleFadingArrowUp/>
                        </div>
                        <div>
                            <p className="text-white">Chamado Atualizado!</p>
                            <p className="text-gray-500">Você atualizou um chamado.</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
